window.addEventListener("message", async e => {
  ...
  if (force_mp4) console.log("[miku] force mp4")
  ...
  (() => {
    ...
    let webvideocaster_tooltipText = "WebVideoCaster";
    ...
    http.onreadystatechange = () => {
      if (http.readyState == 4 && http.status == 200) {
        ...
        if (!fileSize)
          ...
          return console.log(`[miku] Source: ${r[id]} (${return_fileSize})`);
        }
  ...
  async function getConfigMedia(video_config_media, old_url) {
    if (video_config_media)
      ...
      console.log("[miku] universal URL:", normalizedUrl)
  ...
  function updateWebVideoCasterAnchor() {
    ...
    console.log(`[miku] Quality ${r[quality]}p`);
  ...
  function getSourceLocale() {
    try {
      ...
      displayError(`cookies ${linkIssue("deaktiviert", "51#issuecomment-1190684190")}!</code>`);
      throw err;
    }
  }

  function linkIssue(text, issue = "") {
    return console.log('idc lol')
  }
